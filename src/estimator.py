...
def estimator(data):
    ...
    impact_severeCasesByRequestTime = (impact_infectionsByRequestedTime * 0.15)//1
    #This is the estimated number of severe impact positive \n
    # cases that will require hospitalization to recover.
    severeImpact_severeCasesByRequestTime = (severeImpact_infectionsByRequestedTime * 0.15)//1
    ...
    impact_hospitalBedsByRequestedTime = ((hospitalCapacity * 0.35)-impact_severeCasesByRequestTime )//1
    severeImpact_hospitalBedsByRequestedTime = ((hospitalCapacity * 0.35)-severeImpact_severeCasesByRequestTime)//1
    
    #CHALLENGE 3
    
    #This is the estimated number of impact positive \n
    # cases that will require ICU hospitalization to recover.
    impactCasesForICUByRequestTime = (impact_infectionsByRequestedTime * 0.05)//1
    #This is the estimated number of severe positive \n
    # cases that will require ICU hospitalization to recover.
    severeImpactCasesForICUByRequestTime =(severeImpact_infectionsByRequestedTime * 0.05)//1
    
    #This is the estimated number of impact positive cases that will require ventilators.
    impact_casesForVentilatorsByRequestTime = (impact_infectionsByRequestedTime  * 0.02)//1
    #This is the estimated number of severe positive cases that will require ventilators.
    severeImpact_casesForVentilatorsByRequestTime = (impact_infectionsByRequestedTime  * 0.02)//1 


    ##money the economy is likely to lose over the said normal impact period.
    impactDollarInFlight = (impact_infectionsByRequestedTime * \
         data['avgDailyIncomePopulation'] * data['avgDailyIncomeInUSD'] * \
              data['timeToElapse'])//1
    #money the economy is likely to lose over the said severe period.
    severeImpactDollarInFlight = (severeImpact_infectionsByRequestedTime * \
         data['avgDailyIncomePopulation'] * data['avgDailyIncomeInUSD'] * \
              data['timeToElapse'])//1
    Impact = {
